...
`docs/` 안의 보고서 계열 문서(`중간보고서_초안.md`, `최종보고서_보류분.md`,
`references.md`, `experiments_log.md`)는 이 작업과 무관하다. 열지 않아도 된다.
...
| ② 종목 데이터 조회 | `app_ui/tab_data.py` | 없음 |
...
작업 1이 끝난 뒤에 손댄다. 범위를 미리 넓히지 말고, 발표 형식이 정해진 다음에
필요한 만큼만 한다.
