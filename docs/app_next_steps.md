...
**작업 1의 구조(설정 파일 분리)는 2026-09-26에 끝났다** — [config/models.toml](../config/models.toml),
`shared.load_ui_models()`. 남은 것은 회사별 최신 모델 후보를 한 번씩 불러 본 뒤 목록에 넣는 일이다
(아래 "목록은 골라서 넣는다"). 같은 날 claude-sonnet-5를 넣었고, 목록에 없는 모델은 화면의
"직접 입력"으로 쓸 수 있게 했다. gemini-3.8-flash는 Gemini 프로젝트 크레딧 소진(402)으로 확인하지
못했고 gpt-5.6-luna는 temperature=0을 거부해 넣지 않았다(`TODO.md` 앱-15). **Gemini 크레딧 소진은
기본 모델에도 걸릴 수 있으니 시연 전에 확인할 것.** 아래 3절은 착수 전 계획으로 남겨 둔다.
