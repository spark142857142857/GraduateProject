...
같은 문구를 공유하고, 백테스트와 forward도 같은 텍스트를 쓴다. 문구는 `src/prompt.py`
한 곳에서 관리해 양쪽에 동시 반영된다.

관용구가 아니라 실험 목적에 맞춰 넣은 문구가 셋 있다.

| 문구 | 설계 의도 |
|------|----------|
| "기업의 브랜드 이미지·인지도·과거 명성은 **완전히 배제**합니다" | 사전학습된 종목 평판이 판단에 개입하는 것을 차단. cond1 편향 검증(아래 결과 요약)이 겨냥하는 바로 그 경로다 |
...
세 번째 문구는 결과 해석에 직접 관계된다. cond1(종목명만)에서 **Gemma·GPT는 전량 Neutral,
Claude는 응답 거절 27.4%**로 "정보가 없으면 판단하지 않는" 행동을 보였고, 방향성 판단을 낸
것은 flash-lite 하나뿐이었다. 기권이 정당한 선택지로 제시된 설계와 정합하는 관측이다
(다만 문구가 그 행동을 **일으켰다는 인과는 검증하지 않았다** — 프롬프트를 바꾼 대조군이 없다).

#### 알려진 한계

프롬프트를 수정하면 백테스트 전체(4모델 × 5조건 × 3,600콜)를 다시 돌려야 하므로 고치지
않고 한계로 남긴다.

| 한계 | 내용 | 영향 범위 |
|------|------|----------|
| 판단 기준에 **임계치가 없다** | "향후 20거래일 내 주가 상승 예상"이라고만 쓰여 몇 % 이상이 상승인지 정의하지 않는다. Buy와 Neutral의 경계가 모델 재량이다 | 같은 cond4에서도 Buy 건수가 **Claude 58 ↔ Gemini 320으로 5.5배** 벌어진다(720건 중). **모든 조건에 동일하게 적용되므로 조건 간 비교는 영향받지 않으나**, 모델 간 절대 신호 수 비교에는 주의가 필요하다 |
| **confidence 기준이 의도대로 작동하지 않는다** | "50 미만이면 반드시 Neutral"을 명시했으나 temperature=0에서 값이 좁은 대역에 몰린다(flash-lite cond4 std 3.6) | 신뢰도를 성과 지표로 쓰지 않고 보조 정보로만 표시한다. 앱 화면에도 같은 취지의 안내를 띄운다 |
| **reasons 3개 고정** | 근거가 둘뿐인 상황에서도 세 문장을 만들게 강제한다 | 억지 근거가 섞일 수 있어 근거 텍스트를 정량 분석의 근거로 쓰지 않는다 |

사용자가 프롬프트를 바꿔 넣는 기능은 **의도적으로 넣지 않았다.** 프롬프트가 통제 변수이므로
교체를 허용하면 화면에 함께 표시되는 백테스트 성과(그 프롬프트로 얻은 수치)와 신호가 서로
다른 조건의 산물이 된다.
...
- **신호 수준과 포트폴리오 수준은 다른 그림**: cond4는 컨센서스를 이기지만, 실제로 운용하면
  어느 모델도 "실험 20종목 동일가중 보유"(+188.6%)를 넘지 못한다. 다만 낙폭은 줄여준다
