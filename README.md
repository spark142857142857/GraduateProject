...
값이 비면(적자라 PER이 정의되지 않거나 DART 수치를 못 읽은 경우) "0이 아니라 수집하지
못했다는 뜻"이라고 사유를 적고, 내려받는 파일에도 빈 값으로 들어간다는 점을 함께 알린다.
...
받는다. **애널리스트 리포트는 제외한다** — `ensure_reports`가 백테스트 20종목을 건드리지 않아
(그 CSV는 `crawl.py`가 관리하는 실험 입력) 종목에 따라 갱신 기준이 갈리기 때문이다. 화면 전체를
