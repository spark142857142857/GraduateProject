...
공통 조건은 2023-01~2025-12(36개월) 매월 첫 거래일, KOSPI/KOSDAQ 대형주 20종목, 4모델
(temperature=0.0), 20거래일 후 절대·초과 수익률 평가다. 프롬프트 설계와 전체 변수 목록은
[EXPERIMENT_VARS.md](EXPERIMENT_VARS.md) 참고.
...
│   ├── tab_data.py                  # ② 분석 프롬프트 생성·수집 지표 내려받기 (LLM 호출 없음)
...
├── config/
│   └── models.toml                  # 탭① 모델 선택 목록 (한 항목 추가하면 화면에 뜬다)
├── src/
...
탭 3개로 구성된다. **API를 호출하는 것은 탭①의 "분석하기" 버튼 하나뿐**이며, 나머지 두 탭은
LLM을 쓰지 않는다.

| 탭 | 내용 | LLM API |
|----|------|---------|
| ① 개별 종목 분석 | **KRX 상장 보통주 전 종목(약 2,760개)** 중 하나를 골라 오늘 기준 신호 생성. 신호 배지·투자 근거·재무지표(cond2+)·리포트(cond3+)·DART 실적(cond4)·해당 종목 백테스트 성과 | **호출** |
| ② 분석 프롬프트 생성 | 선택 종목의 데이터를 현재 시점으로 수집해 **실험과 같은 형식의 프롬프트**를 만든다. 복사해 ChatGPT·Claude 등 원하는 LLM에 붙여 넣어 쓴다. 수집 지표 요약과 TXT/CSV/JSON/Markdown 내려받기는 옆 칸에 둔다 | 없음 |
| ③ 포트폴리오 백테스트 | 신호대로 운용했을 때의 누적 곡선 + MDD + 연환산 변동성. 거래비용 왕복 0.25% 토글 | 없음 |

①②가 시스템, ③이 검증이다. 백테스트 신호 매트릭스·성과 비교·신호 전이 화면은 **앱 마운트에서
제외**했다 — 인터랙션이 값을 더하지 않아 정지 화면 한 장으로 대체 가능하고, 그 내용은 포스터와
발표 영상이 맡기 때문이다. 모듈 파일은 `app_ui/`에 남겨 도표 캡처용으로 쓴다. 범위 판단의
근거는 [app_scope.md](docs/app_scope.md)에 정리했다.

**탭①의 대상은 백테스트 20종목이 아니라 KRX 상장 보통주 전 종목이다.** 백테스트와 forward는
방법을 검증하는 통제 실험이고 분석 자체는 임의 종목에 적용되어야 하기 때문이다. 20종목 밖은
과거 성과 이력이 없는데, 이는 화면에서 명시한다. 우선주는 DART
재무제표가 보통주 기준 하나뿐이라 PER·시가총액이 어긋나므로 목록에서 제외한다(근거는
[prove.md](docs/prove.md) "각도 4").

**탭①의 모델 목록은 [config/models.toml](config/models.toml)에서 읽는다.** `[[model]]` 항목을
하나 추가하면 코드 수정이나 재시작 없이 목록에 뜬다(첫 항목이 기본 선택). 백테스트를 거친 연구
검증 4모델은 `verified = true`이고, 나중에 추가한 모델은 이름 옆에 "(백테스트 없음)"이 붙으며 과거
성과를 표시하지 않는다. 파일이 없거나 깨지면 4모델로 폴백하고 경고만 띄운다. **알려진 경계**:
호출은 `llm_experiment._provider()`가 접두어로만 회사를 가르므로(gemini·gemma / gpt / claude)
같은 회사의 새 모델은 한 줄로 추가되지만 새 회사나 접두어가 다른 모델(o3 등)은 넣을 수 없다.
앱은 그런 항목을 목록에서 걸러 내고 이유를 보여 준다. 호출이 temperature=0.0 고정이라 이를
거부하는 모델도 쓸 수 없으니 추가하기 전에 한 번 불러 볼 것(실측: gpt-5.6-luna가 거부). 목록
맨 끝의 **"직접 입력"**을 고르면 설정 파일을 고치지 않고 모델 ID를 쳐서 바로 쓸 수 있다. ID는
영문·숫자·`.-_`와 지원 접두어만 받고(결과 저장 경로가 되기 때문), temperature 미지원이나 없는
ID로 실패하면 원문 예외 대신 이유를 알려 준다. 다른 회사 모델은 탭②의 프롬프트를 복사해 쓴다.

탭①에서 생성된 신호는 `results/forward_demo/`로 격리해 정식 평가 표본(`forward_eval.py`)에
섞이지 않게 한다.

**부실한 입력은 감추지 않고 밝힌다.** 전 종목을 열어 둔 이상 대형주만 들어오지 않는다.
값이 비면(적자라 PER이 정의되지 않거나 DART 수치를 못 읽은 경우) 어떤 항목을 수집하지
못했는지와, 모델이 그 값 없이 판단했다(탭②에서는 프롬프트에 N/A로 들어간다)는 점을 알린다.
거래정지·상장폐지 종목은 더 위험하다 — FDR이 마지막 종가를 계속 돌려주므로 값이 채워져
나오는데 그 값이 죽어 있다. 실측(이노벡스 279060)에서 142거래일 연속 거래량 0에 종가가
115원으로 고정돼 있었고, 모델은 그 0을 실제 관측으로 읽어 "변동성이 없다"는 근거를
만들어냈다. 시세 캐시 끝에서 거래량 0이 5거래일 이상 연속되면 두 탭 모두에서 경고한다.
시총 하한으로 종목을 막는 대신 상태를 밝히는 쪽을 택했다.

탭②는 `forward_test.run_forward`와 같은 순서(`get_today_context` → 컨텍스트 빌더 →
`build_prompt`)로 프롬프트를 만든다. 판단 기간·출력 형식 같은 옵션을 두지 않은 것은 실험에서
검증한 프롬프트를 그대로 내기 위해서다. 한 번 수집한 뒤 조건만 바꾸면 다시 수집하지 않고
프롬프트만 새로 그린다. API 키 없이 쓸 수 있고, 앱이 연동하지 않은 회사의 모델에도 쓸 수 있다
(탭①은 `llm_experiment._provider()`의 접두어 분기에 묶여 있다). 애널리스트 리포트는 프롬프트에만
들어가고 내려받는 파일에는 넣지 않는다.

**애널리스트 리포트는 앱에서 네이버 API로 받는다.** 2026년 하반기 네이버가 리서치 페이지를
`finance.naver.com`에서 `stock.naver.com`으로 옮기면서 `crawl.py`가 읽던 목록 표가 사라져, 전
종목이 0건으로 나오게 됐다(실험 데이터는 그 전에 수집을 마쳤다). 새 페이지가 쓰는 JSON API
(`stock.naver.com/api/stockSecurity/researches/v2/company/by-items`)에서 오늘 기준 30일치를
받아 `get_today_context()` 결과의 `recent_reports`만 바꿔 끼운다(`app_ui/shared.py`
`_install_report_source`). `src/`는 동결 상태 그대로이고 **파일은 쓰지 않는다** — 백테스트
20종목의 리포트 CSV는 실험 입력이라 앱이 손대지 않으며, 파일을 거치지 않으므로 20종목도 최신
리포트를 받는다. 앱 프로세스 안에서만 바뀌므로 주간 배치 스크립트에는 영향이 없다.

20종목 밖 종목은 `app_ui/shared.py`의 `register_ticker()`가 종목명과 상장 시장을 `src/`의
레지스트리(`TICKERS`, `KOSDAQ_TICKERS`)에 넣은 뒤 프롬프트를 만든다. 넣지 않으면 티커 코드가
회사명으로, 코스닥 종목이 "상장 시장: KOSPI"로 프롬프트에 들어간다.
